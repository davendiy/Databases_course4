...
create table DWH_DIAGNOSES (
    DWH_DIAGNOSIS_ID number(12),
    DWH_DIAGNOSIS_NAME varchar(256),
    DWH_DIAGNOSIS_DESCRIPTION varchar(1024),
    ESTIMATED_PRICE number(16, 2),
    ESTIMATED_EXEC_TERM number(16),
    constraint dwh_diagnoses_pk primary key (DWH_DIAGNOSIS_ID)
);

create table DWH_DIAGNOSES_ID (
    DWH_DIAGNOSIS_ID number(12),
    DWH_SOURCE_ID number(12),
    EXT_ID varchar(128),
    constraint dwh_diagnoses_id primary key (DWH_DIAGNOSIS_ID, DWH_SOURCE_ID),
    foreign key (DWH_SOURCE_ID) references DWH_SOURCES(SOURCE_ID),
    foreign key (DWH_DIAGNOSIS_ID) references DWH_DIAGNOSES(DWH_DIAGNOSIS_ID)
);

alter table DWH_REPAIR_INVOICES
    add foreign key (DWH_DIAGNOSIS_ID) references DWH_DIAGNOSES(DWH_DIAGNOSIS_ID);
...
CREATE GLOBAL TEMPORARY TABLE LOAD_PRIMARY_INVOICES
(
    INVOICE_DATE DATE,
    DEVICE_MODEL VARCHAR(256),
    DEVICE_STATE VARCHAR(256),
